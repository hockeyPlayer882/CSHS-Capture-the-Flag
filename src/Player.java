...
public class Player extends Sprite{
    ...
    public static void instantiateTeams(){
        ...
        for(int i = 0; i < PlayerLogic.yourTeam.length;i++){
            PlayerLogic.yourTeam[i] = new PlayerLogic(120, i*(Main.frameHeight/5)+(Main.frameHeight/5)/2,youAreRed,90);
            PlayerLogic.enemyTeam[i] = new PlayerLogic(Main.frameWidth-120, i*(Main.frameHeight/5)+(Main.frameHeight/5)/2,!youAreRed,180);
        }
    }
}