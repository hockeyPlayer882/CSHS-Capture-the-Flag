...
public class Sprite {
    // images are rendered differently from polygons (IE: triangles)
    BufferedImage image;
    boolean isImage;
    String polygon;
    Color color;
     ...
     * 
     * @param isImage   true if sprite should be rendered as an image, false
     *                  otherwise
     * @param c         the color of the object you want to render (not used for
     *                  images)
    ...
    public Sprite(boolean isImage, Color c, int x, int y, int size, String imageName) {
        ...
        if (isImage) {
            ...
            // ImageIO code must be sorrounded by a try/catch block, it tries the code in
            // the try, if it fails, it goes to the catch part
            try {
